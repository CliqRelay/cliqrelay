...
Let bindings can also be used to destructure arrays and dictionaries. In this case, the structure of the left-hand side of the assignment should mirror the array or dictionary: With bindings corresponding by position for arrays and by key name for dictionaries. The .. operator can be used once in the pattern to collect the remainder of the array’s or the dictionary’s items.
...
Instead of a string or path, you can also use a module value, as shown in the following example:
