...
export function ManageMemberTeamsSheet({
	...
	const { data: allOrgTeams, isLoading: orgTeamsLoading } =
		authulaClient.organizations.useListOrganizationTeams(orgId, {
			query: { enabled: enabled && isOwner, staleTime: 0 },
		});

	const { data: myTeamsResponse, isLoading: myTeamsLoading } =
	...
	const teams = useMemo(() => {
		...
		return (myTeamsResponse?.teams ?? []).filter(
			(t) => t.organizationId === orgId,
		);
	}, [isOwner, allOrgTeams, myTeamsResponse, orgId]);

	const { data: membershipsRes, isLoading: membershipsLoading } =
	...
	const isLoading =
		enabled && (membershipsLoading || orgTeamsLoading || myTeamsLoading);
