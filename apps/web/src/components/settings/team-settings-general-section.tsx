...
export function TeamSettingsGeneralSection({ team, orgId }: Props) {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        const updated = await authulaClient.organizations.updateOrganizationTeam(orgId, team.id, {
          name: value.name,
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Manage your team's basic settings</p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Team Name</CardTitle>
          <CardDescription>This is the name displayed across your team</CardDescription>
        </CardHeader>
