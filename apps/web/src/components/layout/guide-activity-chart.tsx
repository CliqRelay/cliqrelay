...
export default function GuideActivityChart({ guides }: GuideActivityChartProps) {
    ...
    <Card className="w-full gap-6 py-6">
      <CardHeader className="flex flex-col items-start justify-between gap-3 px-6 sm:flex-row sm:items-center">
        <div className="flex flex-col gap-1">
          <CardTitle className="text-lg font-medium">Guide Creation Activity</CardTitle>
          <div className="flex items-center gap-2">
            <h3 className="text-3xl font-medium text-card-foreground">{totalCreated}</h3>
        ...
        <ChartContainer config={chartConfig} className="h-[300px] w-full">
          <BarChart accessibilityLayer data={monthlyData}>
            ...
            <YAxis
              ...
              allowDecimals={false}
            />
            <Bar dataKey="guides" fill="var(--color-sky-400)" radius={[4, 4, 0, 0]} barSize={20} />
          </BarChart>
        </ChartContainer>
      </CardContent>
    </Card>
