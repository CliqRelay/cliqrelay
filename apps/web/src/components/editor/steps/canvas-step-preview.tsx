...
import { ImageUpIcon, InfoIcon, Loader2Icon, Quote, TriangleAlertIcon } from "lucide-react";
...
export function CanvasStepPreview({ step, onReplaceMedia, isReplacing }: Props) {
  ...
  const alertBackgroundClassName =
    alertBackgroundClassNameMap[canvasContent.type as keyof typeof alertBackgroundClassNameMap] ??
    "";
  const alertForegroundClassName =
    alertForegroundClassNameMap[canvasContent.type as keyof typeof alertForegroundClassNameMap] ??
    "";

  const variant = canvasContent.type === "alert" ? "destructive" : ("default" as const);

  const hasMedia = !!step.mediaAssets?.length;
  const uploadToolbar = onReplaceMedia && !hasMedia && (
    <StepMediaToolbar visible={isReplacing}>
      <StepMediaToolbarButton
        label={isReplacing ? "Uploading…" : "Upload screenshot"}
        disabled={isReplacing}
        onClick={onReplaceMedia}
        icon={isReplacing ? <Loader2Icon className="animate-spin" /> : <ImageUpIcon />}
      />
    </StepMediaToolbar>
  );
  ...
  return (
    <>
      <Alert
        variant={variant}
        className={cn(
          "group relative flex flex-row items-center justify-start",
