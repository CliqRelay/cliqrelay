  ...
  return (
    <>
      <Card
        ...
        tabIndex={0}
        className={cn(
          "group mx-auto h-full w-full max-w-sm surface-card-hover cursor-pointer gap-2 overflow-hidden surface-card rounded-2xl py-0 transition-all duration-200",
      ...
      >
            ...
            {selectable && (
              <RoleGuard minRole={AppUserRole.EDITOR}>
                <Checkbox
                  ...
                  className="size-4 data-[state=checked]:border-primary data-[state=checked]:bg-primary"
        ...
        <CardContent className="flex min-h-0 flex-1 flex-col gap-1 px-5">
          <div className="space-y-1">
            <CardTitle className="line-clamp-2 text-[15px] leading-snug font-semibold">
              {guide.title}
            </CardTitle>
            {guide.description && (
              <CardDescription className="line-clamp-2 text-[13px] leading-relaxed text-muted-foreground">
                {guide.description}
              </CardDescription>
            )}
          </div>
          {guide.creator?.name && (
            <div className="mt-auto flex items-center gap-1 pt-2 text-[11px] text-muted-foreground/50">
              <UserAvatar user={guide.creator} className="size-4" />
              <span className="text-muted-foreground/70">{guide.creator.name}</span>
            </div>
          )}
        </CardContent>
        <CardFooter className="gap-1.5 px-5 pt-0 pb-3">
            ...
            <div className="ml-auto">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  {guide.status === "deleted" && (onRestore || onDeletePermanently) ? (
                    <>
                      {onRestore && (
                        <RoleGuard minRole={AppUserRole.EDITOR}>
                          <DropdownMenuItem
                            className="cursor-pointer gap-2.5 rounded-lg px-3 py-2 text-[13px]"
                            onClick={(e) => {
                              e.stopPropagation();
                              onRestore(guide.id);
                            }}
                      ...
                      {onDeletePermanently && (isCreator || isAdmin) && (
                        <>
                          ...
                          <DropdownMenuItem
                            className="cursor-pointer gap-2.5 rounded-lg px-3 py-2 text-[13px] text-destructive focus:text-destructive"
                            onClick={(e) => {
                              e.stopPropagation();
                    ...
                    <>
                      <RoleGuard minRole={AppUserRole.EDITOR}>
                        ...
                        {(guide.status === "draft" || guide.status === "published") && (
                          <DropdownMenuItem
                            onClick={(e) => {
                              e.stopPropagation();
                              onArchive?.(guide.id);
