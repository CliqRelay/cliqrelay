      ...
      <div className="flex gap-6">
        <div className="flex flex-1 flex-col justify-between">
          <div className="space-y-4">
            <div className="border-muted-background flex w-90 gap-1 rounded-lg border p-1">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="space-y-3 surface-card rounded-2xl p-4">
          ...
          <div className="mt-8 flex justify-center gap-1.5">
            {Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className="h-9 w-9 animate-pulse rounded-[14px] border border-border bg-surface-1"
              />
            ))}
          </div>
        </div>
  ...
  return (
    <div className="dashboard-page__wrapper">
      ...
      <CreateGuideDialog
        ...
        onCreated={(guideId) => navigate({ to: "/dashboard/guides/$guideId", params: { guideId } })}
      />
