        ...
        <div className="min-w-0 flex-1">
          <div className="grid grid-cols-1 items-start gap-3 md:grid-cols-2 lg:grid-cols-3">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="flex flex-col gap-3 surface-card rounded-2xl border border-border p-4"
              >
...
function TrashGuides() {
  ...
  const [deleteDialogGuideId, setDeleteDialogGuideId] = useState<string | null>(null);
  ...
  const handleRestore = async (guideId: string) => {
    try {
      ...
      const newTotalPages = Math.max(1, Math.ceil(Math.max(0, total - 1) / PAGE_SIZE));
  ...
  const confirmPermanentDelete = async () => {
    ...
    const deletedCount = deleteDialogGuideId === "__bulk__" ? selectedIds.length : 1;
    ...
    try {
      ...
      const newTotalPages = Math.max(1, Math.ceil(Math.max(0, total - deletedCount) / PAGE_SIZE));
      if (currentPage > newTotalPages) {
        setCurrentPage(newTotalPages);
      }
    } catch (error) {
      toast.error("Error", {
        description: error instanceof Error ? error.message : "Failed to permanently delete",
      });
    } finally {
            ...
            renderEmpty={() => (
              <Card className="w-full">
                <CardContent className="flex flex-col items-center justify-center py-16">
                  ...
                  <h2 className="mb-2 text-xl font-semibold text-foreground">Trash is empty</h2>
                  <p className="max-w-sm text-center text-sm text-muted-foreground">
                    Deleted guides will appear here and be permanently removed after 30 days.
                  </p>
                </CardContent>
              </Card>
            )}
