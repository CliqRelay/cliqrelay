...
const getActiveTeamIdFromBackground = async (): Promise<string | undefined> => {
  try {
...
export const isActiveTeamCookieChanged = (change: CookieChange): boolean =>
  change.cookie.name === COOKIE_CONSTANTS.activeTeamId.name &&
  !(change.removed && change.cause === "overwrite");
