...
export function ScreenshotDisplay({
  ...
  const containerAspectRatio = hasAspectRatio ? `${viewportWidth} / ${viewportHeight}` : undefined;
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center gap-1.5 rounded-lg border bg-muted/30 py-6">
        ...
        <span className="text-[10px] text-muted-foreground/40">Screenshot unavailable</span>
      </div>
    );
  }
