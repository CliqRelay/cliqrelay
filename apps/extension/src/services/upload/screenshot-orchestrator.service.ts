...
export type ScreenshotUploadOrchestrator = ReturnType<typeof createScreenshotUploadOrchestrator>;
...
export const createScreenshotUploadOrchestrator = (
  ...
  const uploadScreenshotForStep = async (
    ...
    const replaceResponse = await api.uploads.replaceUpload(
      {
        stepId,
        storagePath,
    ...
    return {
      screenshotUrl: replaceResponse.url,
      storagePath,
    };
  };
  ...
  const processCaptureForUpload = async (
    ...
    const {
      webpBlob,
      thumbnailBase64: generatedThumbnail,
      width,
      height,
