...
func InitAuth(envConfig *constants.EnvConfig, authConfig AuthInitConfig) *authula.Auth {
	apiBasePath := "/api/v1"
	...
	plugins := []authulamodels.Plugin{
		...
		jwtplugin.New(types.JWTPluginConfig{
			Enabled: true,
		}),
	...
	plugins = append(plugins, authConfig.AuthulaPlugins...)
