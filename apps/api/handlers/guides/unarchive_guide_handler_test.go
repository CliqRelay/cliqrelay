...
func TestUnarchiveGuideHandler(t *testing.T) {
	t.Parallel()
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedStatus == http.StatusOK {
				mockRepo.On("GetByID", mock.Anything, mock.Anything, guideID).
					...
					Once()
				mockRepo.On("Unarchive", mock.Anything, mock.Anything, guideID).
					Return(&models.Guide{
						ID:        uuid.MustParse(guideID),
						CreatorID: "test-user-123",
						Title:     "Unarchived Guide",
