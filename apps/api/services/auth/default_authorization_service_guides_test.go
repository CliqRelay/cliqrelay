...
var errLookup = errors.New("database is down")
...
func accessibleTeam() *models.Team {
	return &models.Team{
		OrganizationTeam: orgtypes.OrganizationTeam{ID: testTeamID, OrganizationID: testOrgID},
		OwnerID:          "org-owner-1",
	}
}

func newService(t *testing.T, team *models.Team, errLookup error) (*authservice.DefaultAuthorizationService, *tests.MockTeamsService) {
	t.Helper()
	teamsService := new(tests.MockTeamsService)
	teamsService.On("GetAccessibleByUserID", mock.Anything, testActorID, testTeamID).Return(team, errLookup)
